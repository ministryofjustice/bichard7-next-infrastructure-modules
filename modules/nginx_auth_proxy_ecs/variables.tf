...
variable "dns_resolver" {
  description = "The dns resolver hostname for nginx"
  type        = string
}
...
variable "nginx_auth_proxy_ecs_arn" {
  description = "The arn of our NGINX auth"
  type        = string
}

variable "nginx_auth_proxy_ecs_image_url" {
  description = "Our image URL to deploy"
  type        = string
}

variable "nginx_auth_proxy_ecs_image_hash" {
  description = "The hash of our deployable image"
  type        = string
}

variable "nginx_auth_proxy_log_group" {
...
variable "desired_instance_count" {
  description = "How many containers do we want to deploy"
  type        = number
  default     = 1
}
