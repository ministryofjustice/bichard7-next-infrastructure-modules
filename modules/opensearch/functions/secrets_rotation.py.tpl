...
def create_secret(service_client, arn, token):
    ...
    previousPassword = service_client.get_secret_value(
...
def set_secret(service_client, arn, token):
    ...
    previousPassword = service_client.get_secret_value(
        SecretId=arn, VersionStage="AWSCURRENT")
    newPassword = service_client.get_secret_value(
        SecretId=arn, VersionStage="AWSPENDING")

    # HTTP PUT will update the password in the opensearch database
    try:
        url = 'https://${opensearch_custom_domain}/_plugins/_security/api/user/${os_username}'
        headers = urllib3.make_headers(
            basic_auth='${os_username}:'+previousPassword["SecretString"])
...
def test_secret(service_client, arn, token):
    ...
    newPassword = service_client.get_secret_value(
        SecretId=arn, VersionStage="AWSPENDING")
    # curl opensearch domain and see if we are authed
    try:
        headers = urllib3.make_headers(
            basic_auth='${os_username}:' + newPassword["SecretString"])
        url = 'https://${opensearch_custom_domain}/_plugins/_security/api/user/${os_username}'
